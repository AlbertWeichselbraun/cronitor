...
class BorgBackupMonitor(Monitor):
    ...
    def notify(self, force=False):
        ...
        for path in self.borgbackup_base_path.glob('*'):
            ...
            notification_required = any((host for host, last_backups in backups.items()
            ...
            if notification_required or force:
                ...
                msg += [f'- {host}: {", ".join(last_backups)}' for host, last_backups in backups.items()]
        return '\n'.join(msg)

    def parse_borgbackup_output(self, out: str, current_date: datetime = datetime.now()):
        """
        Returns:
